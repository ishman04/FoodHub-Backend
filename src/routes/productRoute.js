...
productRouter.post('/create',isLoggedIn,isAdmin,uploader.single('productImage'), createProd) //multer middleware which handles the file upload process. It parses the incoming request and stores the uploaded file on the server temporarily. The uploader middleware extracts the image file from the request and adds its details to req.file.
